  ...
  const applyProgress = useCallback((p: number) => {
    ...
    if (overlayRef.current) {
      if (c.maskChildren) {
        ...
        // Start text at 75% scale and grow to 100% as the window expands
        overlayRef.current.style.transform = `scale(${0.75 + 0.25 * e})`;
      } else {
  ...
  const media =
    mediaType === 'video' ? (
      ...
      <div 
        ref={mediaRef} 
        className="scroll-expand__media" 
        style={{ backgroundColor: src || '#ffffff' }} 
      />
    ) : (
      ...
      <div ref={trackRef} className="scroll-expand__track">
        <div ref={stageRef} className="scroll-expand__stage">
          ...
          <div ref={frameRef} className="scroll-expand__frame">
            {media}
            {mediaType !== 'color' && <div ref={scrimRef} className="scroll-expand__scrim" />}
            {children ? (
